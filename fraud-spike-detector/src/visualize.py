...
def plot_fraud_timeline(full_df_path: str = "data/transactions_full.csv",
                        output_path: str = "reports/fraud_timeline.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate fraud timeline: %s", e)
        print(f"  FAILED: fraud_timeline — {e}")


def plot_confusion_matrices(pred_df_path: str = "data/predictions_test.csv",
                            output_path: str = "reports/confusion_matrices.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate confusion matrices: %s", e)
        print(f"  FAILED: confusion_matrices — {e}")


def plot_roc_curves(pred_df_path: str = "data/predictions_test.csv",
                    output_path: str = "reports/roc_curves.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate ROC curves: %s", e)
        print(f"  FAILED: roc_curves — {e}")


def plot_pr_curves(pred_df_path: str = "data/predictions_test.csv",
                   output_path: str = "reports/pr_curves.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate PR curves: %s", e)
        print(f"  FAILED: pr_curves — {e}")


def plot_feature_importance(pred_df_path: str = "data/predictions_test.csv",
                            metadata_path: str = "models/metadata.json",
                            output_path: str = "reports/feature_importance.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate feature importance: %s", e)
        print(f"  FAILED: feature_importance — {e}")


def plot_cost_comparison(metrics_path: str = "reports/metrics.csv",
                         output_path: str = "reports/cost_comparison.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate cost comparison: %s", e)
        print(f"  FAILED: cost_comparison — {e}")


def plot_metrics_radar(metrics_path: str = "reports/metrics.csv",
                       output_path: str = "reports/metrics_radar.png") -> None:
    ...
    try:
        ...
        log.error("Failed to generate metrics radar: %s", e)
        print(f"  FAILED: metrics_radar — {e}")


def plot_detection_by_spike_type(pred_df_path: str = "data/predictions_test.csv",
                                 output_path: str = "reports/detection_by_type.png") -> None:
    """Per-spike-type detection rate heatmap across all detectors."""
    try:
        df = pd.read_csv(pred_df_path)
        det_names = _get_detector_columns(df)

        if "spike_type" not in df.columns:
            print("  Skipped: spike_type column not found")
            return

        fraud_df = df[df.window_label == 1].copy()
        if len(fraud_df) == 0:
            print("  Skipped: no fraud windows in test set")
            return

        spike_types = [st for st in fraud_df["spike_type"].unique() if st != "none"]
        if not spike_types:
            print("  Skipped: no labeled spike types")
            return

        detection_rates: dict[str, list[float]] = {}
        for name in det_names:
            pred_col = f"{name}_pred"
            rates: list[float] = []
            for st in spike_types:
                mask = fraud_df["spike_type"] == st
                if mask.sum() == 0:
                    rates.append(0.0)
                else:
                    rates.append(float(fraud_df.loc[mask, pred_col].mean()))
            detection_rates[_display_name(name)] = rates

        rate_df = pd.DataFrame(detection_rates, index=spike_types)

        fig, ax = plt.subplots(figsize=(10, 4))
        sns.heatmap(rate_df.T, annot=True, fmt=".2f", cmap="RdYlGn", vmin=0, vmax=1,
                    ax=ax, annot_kws={"fontsize": 12, "fontweight": "bold"},
                    linewidths=1, linecolor="#2a2a3a")
        ax.set_title("Detection Rate by Spike Type", fontsize=14, fontweight="bold")
        ...
        log.error("Failed to generate detection by type: %s", e)
        print(f"  FAILED: detection_by_type — {e}")
